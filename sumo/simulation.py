...
# point to your SUMO installation
...
# wrap Quart in Socket.IO ASGI middleware
...
def update_cfg_with_routes_sync(cfg_file: str, route_files: list[str]) -> None:
    ...
    inp  = root.find("input")
    if inp is None:
        raise ValueError("No <input> element in config")
    rf = inp.find("route-files")
    if rf is None:
        raise ValueError("No <route-files> element in config")
...
if __name__ == "__main__":
    ...
    # serve the ASGI app that combines Quart + Socket.IO
